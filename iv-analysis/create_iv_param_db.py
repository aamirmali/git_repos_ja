...
if __name__ == '__main__':
    ABS=ABS_param()
    db=sqlite3.connect('/home/jappel/iv_database/iv_db_complete',timeout=30.0)
    cursor=db.cursor()
    ...
    print len(datedirs1)
    print len(datedirs2)
    datedirs_all=np.append(datedirs1,datedirs2)
    print len(datedirs_all)
    jobs = []


    p1 = multiprocessing.Process(target=db_datedir,args=(datedirs_all,ABS,cursor))
